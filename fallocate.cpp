...
bool prepare_fallocate_wrapper()
{
	...
	{
		CloseHandle(cur_token); // I'd have used ON_BLOCK_EXIT, but want to keep
	...
	if(!AdjustTokenPrivileges(
		   cur_token, FALSE, &new_tp, sizeof(TOKEN_PRIVILEGES), NULL, NULL))
	{
...
int fallocate_wrapper(handle_t hndl, long long int size_to_reserve)
{
	...
	if(!SetFilePointerEx(hndl, zero, &old_pos, FILE_CURRENT)) return -1;

	// Movie file position to the new end. These calls do NOT result in the
	// actual allocation of new blocks, but they must succeed.
	LARGE_INTEGER new_pos = {0};
	new_pos.QuadPart	  = size_to_reserve;
	if(!SetFilePointerEx(hndl, new_pos, NULL, FILE_END)) return -1;
	if(!SetEndOfFile(hndl)) return -1;

	// Try to use the SetFileValidData call
	if(SetFileValidData(hndl, size_to_reserve) != 0) return 0; // Success!

	// Bummer. Can't expand the file this way - now try sparse files
	DWORD temp = 0;
	// Mark the file as sparse.
	if(DeviceIoControl(hndl, FSCTL_SET_SPARSE, NULL, 0, NULL, 0, &temp, NULL)
	   != 0)
	{
	...
	// allocation.
	if(!SetFilePointerEx(hndl, minus_one, NULL, FILE_END)) return -1;
	char initializer_buf[1] = {1};
	DWORD written			= 0;
	if(!WriteFile(hndl, initializer_buf, 1, &written, NULL)) return -1;
...
int fallocate_wrapper(handle_t hndl, long long int size_to_reserve)
{
	...
	if(fallocate64_raw(hndl, 0, cur_pos, size_to_reserve) == 0) return 0;

	// No such luck :( Use good old lseek64.
	if(lseek64(hndl, cur_pos + size_to_reserve - 1, SEEK_SET) == -1) return -1;
	// Really force the file write.
	const char* one = "O";
	if(write(hndl, one, 1) == -1) return -1;
